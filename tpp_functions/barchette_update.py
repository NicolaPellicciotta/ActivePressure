...
def arrange_many_barchette(vela_size=array([2.,15.,10.]),support_size = array([2.,2.,3.]),Dr=65,speed=100.,xyres=0.25,zres=0.9):
...
def make_barchetta_double(vela_size=array([2.,15.,5.]),support_size = array([2.,2.,2.]),R_vela=None,speed=100.,xyres=0.25,zres=0.9):
    ...
    dondolo_1= make_dondolo(rstart=R_dondolo-dondolo_size[2],R=R_dondolo,speed=speed,xyres=xyres,zres=zres,width=dondolo_size[1],phi_max=20.)
...
def make_barchetta(vela_size=array([2.,20.,5.]),support_size = array([2.,2.,2.]),inverted=True ,R_vela=None,speed=100.,xyres=0.25,zres=0.9):
...
#    fillcol.shift([0,0,-( ((int(nl)+h_corr)*zres) -(height-(zres/2)) )]) # this should place the channel at the right height
    ...
    if R_vela==None:
        ...
        if inverted ==True:  # make vela longer
            vela_size_offset=vela_size+array([0,0,2*zres])
